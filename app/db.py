...
async def set_reminder_set(reminder_details: dict):
    pool = await init_db_pool()
    async with pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            await cur.execute(
                "INSERT INTO reminder (title, datetime, notes) VALUES (%s, %s, %s)",
                (reminder_details["title"], reminder_details["datetime"], reminder_details["notes"])
            )


async def get_reminder_set(start_date: str, end_date: str):
    pool = await init_db_pool()
    async with pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:

            query = """
                SELECT title, notes, datetime, status 
                FROM reminder 
